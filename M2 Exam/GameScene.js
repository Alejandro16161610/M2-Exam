...
export default class Gamescene extends Phaser.Scene{
    ...
    preload(){
      ...
      // Goods
      // Coin
      this.load.image('C1', 'GoodSprites/Coin1.png');
      this.load.image("C2", 'GoodSprites/Coin2.png');
      this.load.image("C3", 'GoodSprites/Coin3.png');
      this.load.image("C4", 'GoodSprites/Coin4.png');
    }
...
function onHit2(Play,enemies,damage){
  enemies.destroy();
  this.soundplay = Phaser.Math.Between(1, 3);
  if (this.soundplay == 1){
  this.ExSound1.play();
...
this.physics.add.overlap(Player, Vigral, onHit2, null, this);
this.physics.add.overlap(Player, Houldrant, onHit2, null, this);
this.physics.add.overlap(Player, Shetrant, onHit2, null, this);

this.physics.add.overlap(Player, Coin, onHit3, null, this);
//------------------------------------
    }
...
if (Health <= 0){
  ...
  this.scene.start('GameOver');
      ...
      function CreateGoods(Gi){
        if (Gi == 1){
      var x = Phaser.Math.Between(30, 755);
      var y = -50;
      var Goods = Coin.create(x, y, 'Coin');
      Goods.setScale(0.05);
      Goods.setVelocityY(150);
      Goods.anims.play('CoinSpin', true);
        }
        else if (Gi == 2){
          var x = Phaser.Math.Between(30, 755);
          var y = -50;
          var newEnemy2 = Coin.create(x, y, 'Coin');
          newEnemy2.setScale(0.05);
          newEnemy2.setVelocityY(150);
          newEnemy2.anims.play('CoinSpin', true);  
        }
      }
...
if (this.time.now > this.Spawned + enemydelay){
  this.Ds = Phaser.Math.Between(1,2);
  if (this.Ds == 1){
